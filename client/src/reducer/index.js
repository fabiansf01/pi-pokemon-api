...
// *******  REQUERIMIENTO MATIAS **************************** //
    case actionTypes.GET_POKEMON_MITIPO: {
      const type = action.payload; 
      if (type === "default")
        return {
          ...state, pokemons: state.allPokemons,filtered: state.allPokemons, };
      else { let pokemonsFiltered = state.allPokemons?.filter((pokemon) => {
          return pokemon.types.includes(type);   });
        return {...state, pokemons: pokemonsFiltered,filtered: pokemonsFiltered, };
      }
    }
